void webPageConfig() { 
  ...
  CP("<p><b> Stromverbrauchs-Einheiten&aumlhler-Typ:</b></p>");
  CP(" <form action=\"/get\" method =\"GET\">");
  CP("<p> <label><input type=radio id=EHZ1 name=EHZType value=1 " + EStr[1]+ "> Einrichtungsz&aumlhler</label></p>");
  CP("<p> <label><input type=radio id=EHZ2 name=EHZType value=2 " + EStr[2]+ "> Zweirichtungsz&aumlhler</label> </p>");  
  
  CP(" <label for=EHZ_BaudrateLabel><b>Baudrate EHZ:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</b></label>");
  ...
  CP("<label for=SendIR_Label> Send Command an EHZ</label>");
