...
int InverterType = 1;                 // 1 = Soyosource, 2 = GoodWe; 3 = Growatt;  EEPROM Adr 1+2 
...
#define CP client.println             // CP  für Kurzscheibweise von client.print
...
double T1Wh = -2, SumWh = -2, Watt = -2;  // Vorgabewerte zum Erkennen von Falsch- / Nichtlesen
...
#define MaxArray      500           // Maximale Anzahl von Bytes die von IR gelesen und gespeichert werden können
...
// Pins konfigurieren
  ...
  digitalWrite(RS485_Tog_Pin,HIGH);  // Pin auf High für TX
...
OBISHandler OBISHandlers[] = {
  ...
  {{ 0x01, 0x00, 0x10, 0x07, 0x00, 0xff }, &PowerW},       /*   1-  0: 15.  7.0*255 (Watt) */
  {{ 0, 0 }}
};
  ...
  if (Serial2.available()) { // neue IR-Daten vorhanden
    ...
    if (ReadIRData()) {       // Auslesen
     digitalWrite(OB_LED_Pin, LEDAUS); 
      NetPowerAct = Get_Cur_Power(1); // 1 für Aktuelle Wirkleistung (Netzbezug) auswerten
      if (Parsing_Success) { // Wattwerte wurde erfolgreich ermittelt
        //digitalWrite(IR_Green_Pin, LEDAN); 
        Print_Power(NetPowerAct); // Daten erfolgreich gelese
        lastokSMLReading = millis();
        long restzeit = WaitInverterPwr - (millis());
        //Serial.println("Noch warten: " + String(restzeit) + " ms.");
      }
      else { // Parsing fehler Grün aus und 2 x Rot blinken
        Parsingfehler();        
      }        
      ConsmptTot = Get_Cur_Power(2); // 2 für Gesamtzählerstand auswerten
      if (Parsing_Success) {
        RPRINT("Zählerstand: ");
      ...
      }   // Ende Parsing erfogreich für Gesamtzählerstand
...
void ChangeInverterPower(long Power) {
  if (Power <= 0) {                                                    // bei 0 Verbrauch Leistung reduzieren
...
long Get_Cur_Power(int DefNr)  //1 = Wirkleistung, 2 = Gesamtverbrauch
{
  int i = 0, iHandler = 0;
  unsigned char c;
  sml_states_t s;
  for (i = 0; i < IR_Dataset_length; ++i) {
    ...
    if (s == SML_FINAL) {
      ...
      if(DefNr==1) return Watt;   // Wirkleistung zurück
      if(DefNr==2) return SumWh;  // Gesamtverbrauch zurück
    }
  }
}

/*
long Get_Cur_Power(int DefNr) {
  Hd_lgh = HdStr[DefNr].length() / 2;          // 18 Header Länge, die erkannt werden muss
  ...
  Hd_found = false;
  Parsing_Success = false;
  DPRINTLN(IR_Dataset_length);
  DPRINTLN("Auswertung beginnt... ");
  DPRINTLN("Byte HexVal");
  int Correct_ID = 0; // Zähler für gesuchte ID: Hd1[Correct_ID]
  ID_act = 0;         //Zähler für IR Empfangs Array SID[ID_act]
  while ((ID_act <= IR_Dataset_length) and (not(Parsing_Success))) { // gehe durch alle Empfangenen Daten, solange der Power noch nicht ermittelt ist (Parsing_Success)
    DPRINT(ID_act);
    DPRINT("  ");
    DPRINT(SID[ID_act], HEX);
    if (SID[ID_act] == HdAtPos(HdStr[DefNr], Correct_ID) or (HdAtPos(HdStr[DefNr], Correct_ID) == 0xFF)) { //    Hd1[Correct_ID]) {
      Correct_ID++;
      DPRINTLN("  Korrekter Headerwert, Position = " + String(Correct_ID) + " ");
    }
    else {
      Correct_ID = 0;
      DPRINTLN();
    }
    if (ID_act >= IR_Dataset_length) Send_complete = false; // Abbruch

    if (Correct_ID == Hd_lgh) { //Header gefunden!!
      DPRINTLN(); DPRINTLN("Header gefunden!"); DPRINTLN();
      Hd_found = true; // header gefunden true
    }
    ID_act++; // Zeiger einen weiter setzen

    if (Hd_found) { // Power Werte auslesen
      CP_lgth = SID[ID_act] - Subtr[DefNr]; // Anzahl Byres die zu lesene sind
      DPRINTLN("Die  " + String(CP_lgth) + " folgenden Byte bilden den Wert der aktuellen Wirk-Leistung");
      ID_act++; // Zeiger einen weiter setzen
      //Berechnung der Power aus den nächsten 1, 2, oder 3 Bytes
      if (CP_lgth == 1) Power = long(SID[ID_act]) / PowerTeiler[DefNr];
      if (CP_lgth == 2) Power = (long(SID[ID_act]) * long(0xFF) + long(SID[ID_act + 1])) / PowerTeiler[DefNr];
      //if(CP_lgth==3) Power=(long(SID[ID_act])*long(0xFFFF)+long(SID[ID_act+1])*long(0xFF)+long(SID[ID_act+2]))/(PowerTeiler);
      if (CP_lgth == 3) Power = (long(SID[ID_act + CP_lgth - 3]) * long(0xFFFF) + long(SID[ID_act + CP_lgth - 2]) * long(0xFF) + long(SID[ID_act + CP_lgth - 1])) / (PowerTeiler[DefNr]); // nur die letzten 3 byte auslesen
      if (CP_lgth > 3) Power = (long(SID[ID_act + CP_lgth - 4]) * long(0xFFFFFF) + long(SID[ID_act + CP_lgth - 3]) * long(0xFFFF) + long(SID[ID_act + CP_lgth - 2]) * long(0xFF) + long(SID[ID_act + CP_lgth - 1])) / (PowerTeiler[DefNr]); // nur die letzten 3 byte auslesen
      Hd_found = false;
      cnt = 0;
      Parsing_Success = true;
      return Power;
    } // Ende Hd_found Berechnung und Ausgabe, wenn gefunden
  }   // Ende Schleife durch empfangene Daten
}         // Ende Funktion
*/
  ...
  if (client) {                                // Falls sich ein neuer Client verbindet,
    ...
    while (client.connected() && (currentWebReqTime - previousWebReqTime <= WebReqTimeoutTime)) {  // loop while the client's connected
      ...
      if (client.available()) {                // Fall ein Byte zum lesen da ist,
        ...
        else if((header.indexOf("GET /get?InvType")>=0) and islocalClient()) ConfigSave();   // Konfigurationswerte der Konfigseite speichern
...
void ConfigSave() {
  ...
  CP("<body> <h3>Daten gespeichert</h3> ");
  ...
  if(val<1 or val>32000) CP("<p>Inverterleistung"+ t + "</p>"); 
  ...
  if(val<1 or val>1000) CP("<p>Minimalwert f&uuml;r Netzbezug"+ t + "</p>"); 
  else MinNetInput=val;

  WriteConfigToEEPROM();
  
  CP("<br>");
  CP("<p><a href=\"/\"><button>Startseite</button></a></p>");
  CP("</body>  </html>");
}
